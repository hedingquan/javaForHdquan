...
public class PermissionController extends NoPermissionException{
	...
	@RequiresPermissions("query:permission")
	...
	@RequiresPermissions("query:permission")
	public void permissionByDepartmentdatagrid(HttpServletRequest request,HttpServletResponse response,String number1) throws IllegalAccessException, InvocationTargetException, UnsupportedEncodingException
	{	
		 request.setCharacterEncoding( "utf-8" );
		 response.setCharacterEncoding("utf-8");
		 response.setHeader("Content-Type" , "text/html");
		 List<Permission> permission = PermissionService.getPermissionByDepartment(Integer.valueOf(number1));
		WriteJson.writeJson(permission,response);
	}
	
	@RequestMapping("getPermissionByUserGroup.action")
	@RequiresPermissions("query:permission")
	public void getPermissionByUserGroup(HttpServletRequest request,HttpServletResponse response,String usGId) throws IllegalAccessException, InvocationTargetException, UnsupportedEncodingException
	{	
		 request.setCharacterEncoding( "utf-8" );
		 response.setCharacterEncoding("utf-8");
		 response.setHeader("Content-Type" , "text/html");
		 List<Permission> permission = PermissionService.getPermissionByUserGroup(usGId);
		WriteJson.writeJson(permission,response);
	}
