...
public class UserGroupController extends NoPermissionException{
	...
	@RequiresPermissions("query:userGroup")
		...
		response.setCharacterEncoding("utf-8");
		  ...
		  for(UserGroup ug:userGroup)
		  {
			  ...
			  if(lookusers!=null&&"true".equals(lookusers))
			  {
				...
				attributes.put("department_number1",ug.getDepartment().getNumber1());
	...
	@RequiresPermissions("add:userGroup")
	...
	@RequiresPermissions("delete:userGroup")
	...
	@RequiresPermissions("update:userGroup")
    ...
    if(permission!=null){
    	List<Role> userGroupRoles = roleService.getUserGroupRoles(request.getParameter("id"));
    	...
    	for(String id:permission.split(","))
		{		
		 Permission permission2 = permissionService.getPermission(id);
		 Role role=null;
		try {
