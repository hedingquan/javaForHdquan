...
public class PointController {
	...
	public void Noscore_point(HttpServletRequest req,HttpServletResponse res) throws IOException, ServletException
	{
		...
		{
			if("End_Working".equals(apply.getApply_state()))
			{
	...
	public void My_point(HttpServletRequest req,HttpServletResponse res) throws IOException, ServletException
	{
		...
		List<Point> myPoint = pointService.getMyPoint(User.getUser_ID());
		for(Point point:myPoint)
		{
			for(Work work:workService.MyWork(point.getUser_id2()))
			{
				if(work.getWork_content()!=null)
				for(String name:work.getWork_content().split(","))
				{
					if(name.equals(user_name))
					{
						point.setUser_id1(work.getWork_title());
					}
				}
			}
		}
		req.setAttribute("point", myPoint);
	...
	public void add_point(HttpServletRequest req,HttpServletResponse res,String work_id,String point_score) throws IOException, ServletException
	{
		...
		Point.setUser_id2((userService.getBuser(workService.getWork(work_id).getUser_id())).getUser_ID());
		Point.setPoint_time(new SimpleDateFormat("MM/dd/yyyy HH:mm:ss").format(new Date()));
		Point.setPoint_score(Integer.valueOf(point_score));
